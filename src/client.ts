...
import {
  ...
  AccountOpenParams,
  AccountOpenResponse,
  AccountRetrieveBalanceHistoryParams,
...
import { Corporate, CorporateOnboardParams, CorporateOnboardResponse } from './resources/corporate/corporate';
...
import {
  ...
  TransactionInitiateDisputeParams,
...
export interface ClientOptions {
  /**
   * Defaults to process.env['GARBAGE_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['GARBAGE_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['GARBAGE_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Garbage API.
 */
export class Garbage {
   ...
   * API Client for interfacing with the Garbage API.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['GARBAGE_API_KEY'] ?? null]
   * @param {string} [opts.baseURL=process.env['GARBAGE_BASE_URL'] ?? https://b4d87a72-0ae6-4d06-b803-cbc895324996.mock.pstmn.io] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('GARBAGE_BASE_URL'),
    apiKey = readEnv('GARBAGE_API_KEY') ?? null,
    ...opts
  }: ClientOptions = {}) {
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://b4d87a72-0ae6-4d06-b803-cbc895324996.mock.pstmn.io`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? Garbage.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('GARBAGE_LOG'), "process.env['GARBAGE_LOG']", this) ??
      defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://b4d87a72-0ae6-4d06-b803-cbc895324996.mock.pstmn.io';
  }
  ...
  static GarbageError = Errors.GarbageError;
...
Garbage.Users = Users;
Garbage.Accounts = Accounts;
Garbage.Transactions = Transactions;
Garbage.AI = AI;
Garbage.Corporate = Corporate;
Garbage.Web3 = Web3;
Garbage.Payments = Payments;
Garbage.Sustainability = Sustainability;
Garbage.Marketplace = Marketplace;
Garbage.Lending = Lending;
Garbage.Investments = Investments;
Garbage.System = System;

export declare namespace Garbage {
